...
public class ConstraintExceptionHandler {
    ...
    @Value("${person.crayon.tool.exception.handler.data-key:fields}")
    ...
    public ApiResponse handle(ConstraintViolationException e) {
        ...
        e.getConstraintViolations().forEach(err -> res.add(new ParamError(((FieldError) err).getField(), err.getMessage())));
